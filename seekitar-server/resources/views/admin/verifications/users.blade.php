        ...
        <div class="card-body py-3 px-4">
            <div class="d-flex align-items-start gap-3">
                ...
                <p class="mb-0 fs-3">
                    ...
                    titik domisilinya, lalu satu klik Setujui atau Tolak dengan alasan.
                </p>
            </div>
        </div>
    </div>
            ...
            <table class="table table-hover mb-0 align-middle">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="text-nowrap">Domisili</th>
                        <th scope="col" class="text-nowrap">Diajukan</th>
                    </tr>
                </thead>
                <tbody>
                @forelse ($pending as $user)
                    @php
                        /*
                         * Dihitung sekali; dipakai warna baris DAN lencana.
                    ...
                    <tr @class(['table-danger' => $lewatSla, 'cursor-pointer'])
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="lh-sm">
                                    ...
                                    @if ($user->rejected_at)
                                        <div class="text-danger mt-1" style="font-size: 12px;" title="{{ $user->rejected_reason }}">
                                            Pengajuan ulang — pernah ditolak {{ $user->rejected_at->diffForHumans() }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        {{-- Domisili diringkas: alamatnya dibandingkan dengan
                             titiknya DI DALAM modal, bukan di kolom sempit. --}}
                        <td>
                            <div class="lh-sm">
                                <div>{{ Str::limit($user->address ?? '—', 32) }}</div>
                                @if ($user->latitude !== null)
                                    <div class="font-monospace text-muted" style="font-size: 12px;">
                                        {{ \App\Support\Angka::desimal($user->latitude, 5) }}, {{ \App\Support\Angka::desimal($user->longitude, 5) }}
                                    </div>
                                @else
                                    <div class="text-danger" style="font-size: 12px;">Tanpa titik peta</div>
                                @endif
                            </div>
                        </td>
