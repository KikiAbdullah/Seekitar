...
async function startSocket() {
    if (state.connecting) {
        ...
        console.log(
            "[WA] ⏳ startSocket dipanggil saat connecting — dilewati."
        );
        return;
    }
    ...
    try {
        ...
        // eslint-disable-next-line no-console
        console.log(
            `[WA] 🔌 Memulai sesi Baileys (versi ${BAILEYS_VERSION.join(
                "."
            )}) — folder: ${SESSION_DIR}`
        );
        ...
        sock.ev.on("connection.update", (update) => {
            ...
            if (qr) {
                ...
                QRCode.toDataURL(qr, { width: 320, margin: 1 })
                    .then((url) => {
                        if (state.lastQr === qr) state.lastQrDataUrl = url;
                    })
                    .catch(() => {});
                // eslint-disable-next-line no-console
                console.log(
                    `[WA] 🟢 QR BARU tersedia (${new Date().toLocaleTimeString()}) — scan dari panel admin.`
                );
            }
            ...
            if (connection === "close") {
                ...
                // eslint-disable-next-line no-console
                console.log(
                    `[WA] ⚠️ Koneksi tertutup (kode ${
                        code ?? "?"
                    }) — percobaan ke-${state.failedAttempts}${
                        state.connecting ? "" : ""
                    }.`
                );

                if (
                    state.failedAttempts >= AUTO_RESET_AFTER &&
                    hasStoredSession() &&
                    !state.autoResetDone
                ) {
                    ...
                    // eslint-disable-next-line no-console
                    console.log(
                        "[WA] 🧹 Sesi tersimpan bermasalah — reset folder sesi, QR baru akan muncul…"
                    );
                    try {
...
function ensureConnected() {
    if (!isSocketOpen()) {
        const err = new Error(
            "WhatsApp belum tersambung. Scan QR di panel admin dulu."
        );
        err.status = 409;
        throw err;
    }
...
async function sendText(phone, text) {
    ...
    // eslint-disable-next-line no-console
    console.log(
        `[WA] 📤 Kirim pesan → ${phone} (${new Date().toLocaleTimeString()})`
    );
    ...
    const timeout = new Promise((_, reject) => {
        setTimeout(() => {
            reject(
                Object.assign(
                    new Error(
                        "Waktu kirim habis — koneksi WhatsApp tidak merespons."
                    ),
                    { status: 504 }
                )
            );
...
app.get("/api/status", (req, res) => {
    ...
    const qrFresh =
        state.lastQr &&
        state.lastQrAt &&
        Date.now() - state.lastQrAt < QR_TTL_MS;
    // eslint-disable-next-line no-console
    console.log(
        `[WA] 📊 /api/status → online=${state.online} phone=${
            state.phone || "-"
        } qr=${qrFresh ? "ADA" : "-"} connecting=${state.connecting}`
    );
    res.json({
        success: true,
        data: {
...
app.get("/api/qr", async (req, res) => {
    ...
    if (state.online) {
        return res.json({
            success: true,
            data: { qr: null, online: true, message: "Sudah tersambung." },
        });
    }
    ...
    try {
        ...
        if (!state.lastQrDataUrl) {
            state.lastQrDataUrl = await QRCode.toDataURL(state.lastQr, {
                width: 320,
                margin: 1,
            });
        }
        res.json({
            ...
            data: {
                ...
                expires_in: Math.max(
                    0,
                    Math.round(
                        (QR_TTL_MS - (Date.now() - state.lastQrAt)) / 1000
                    )
                ),
            },
        });
...
app.post("/api/logout", async (req, res) => {
    // eslint-disable-next-line no-console
    console.log(
        `[WA] 🔌 /api/logout dipanggil (${new Date().toLocaleTimeString()})`
    );
    try {
        if (state.socket) {
            try {
        ...
        res.json({
            success: true,
            message: "Sesi WhatsApp dicabut. Scan QR untuk menyambung ulang.",
        });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
app.post("/api/reset", async (req, res) => {
    // eslint-disable-next-line no-console
    console.log(
        `[WA] 🧹 /api/reset dipanggil (${new Date().toLocaleTimeString()})`
    );
    try {
        if (state.socket) {
            try {
        ...
        res.json({
            success: true,
            message: "Sesi di-reset. QR baru akan segera muncul — scan ulang.",
        });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
app.post("/api/send", async (req, res) => {
    ...
    if (!to || !text) {
        return res.status(422).json({
            success: false,
            message: "Parameter `to` dan `text` wajib.",
        });
    }
    try {
...
app.get("/healthz", (req, res) =>
    res.json({ success: true, data: { online: state.online } })
);
...
function startRedisSubscriber() {
    if (!REDIS_URL) {
        // eslint-disable-next-line no-console
        console.log(
            "[WA] 🟠 REDIS_URL kosong — jalur cepat socket NONAKTIF; pakai HTTP biasa."
        );
        return;
    }
    ...
    // eslint-disable-next-line no-console
    console.log(
        `[WA] 🔵 Menghubungkan ke Redis: ${REDIS_URL.replace(
            /:[^:@/]+@/,
            ":*****@"
        )}`
    );
    ...
    sub.on("ready", () => {
        sub.subscribe(WA_CHANNEL_SEND, (err) => {
            if (err) {
                ...
                // eslint-disable-next-line no-console
                console.error(
                    `[WA] ❌ Gagal subscribe ${WA_CHANNEL_SEND}:`,
                    err.message
                );
            } else {
                ...
                // eslint-disable-next-line no-console
                console.log(
                    `[WA] 🔵 Redis SUBSCRIBED ke channel "${WA_CHANNEL_SEND}" — jalur cepat siap.`
                );
            }
        });
    });
    ...
    sub.on("message", async (channel, message) => {
        ...
        // eslint-disable-next-line no-console
        console.log(
            `[WA] 📨 Pesan dari Redis channel "${channel}" diterima (${new Date().toLocaleTimeString()}).`
        );
        ...
        try {
            ...
            // eslint-disable-next-line no-console
            console.error(
                `[WA] ❌ Hasil kirim via Redis → ${to} (ok=false): ${err.message}`
            );
        }
    });
...
app.listen(PORT, HOST, () => {
    logger.info(
        { port: PORT, host: HOST },
        "seekitar whatsapp gateway listening"
    );
    // eslint-disable-next-line no-console
    console.log("==========================================================");
    console.log(
        `[WA] 🚀 Seekitar WhatsApp Gateway dimulai (PID ${process.pid})`
    );
    ...
    console.log(
        `[WA] 🔑 Token: ${
            BAILEYS_TOKEN
                ? "TERSET (auth aktif)"
                : "KOSONG (tanpa auth — hanya dev!)"
        }`
    );
    console.log(
        `[WA] 🗂️  Folder sesi: ${SESSION_DIR}${
            hasStoredSession()
                ? " (ADA sesi tersimpan — akan auto-connect)"
                : " (kosong — butuh scan QR)"
        }`
    );
    console.log(
        `[WA] 📡 Baileys: ${
            require("@whiskeysockets/baileys/package.json").version
        } (dipin ${BAILEYS_VERSION.join(".")}, tanpa fetch GitHub)`
    );
    console.log(
        `[WA] ⚡ Reconnect ${RECONNECT_DELAY_MS / 1000}s · QR TTL ${
            QR_TTL_MS / 1000
        }s · Auto-reset sesi setelah ${AUTO_RESET_AFTER}x putus`
    );
    console.log("==========================================================");
    startSocket();
