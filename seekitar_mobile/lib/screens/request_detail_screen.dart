...
class _RequestDetailScreenState extends State<RequestDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final results = await Future.wait([
        _api.getRequest(reqId),
        _api.getRequestOffers(reqId),
      ]);
      if (mounted) setState(() {
        _detail = results[0];
        _offers = results[1];
  ...
  Future<void> _sendOffer() async {
    ...
    if (ok == true && priceCtrl.text.isNotEmpty) {
      try {
        await _api.createOffer(_id, {'price': double.parse(priceCtrl.text), 'notes': notesCtrl.text.isNotEmpty ? notesCtrl.text : null});
        _load();
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Penawaran terkirim!')));
