...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadStores() async {
    setState(() { _loadingStores = true; _storesError = null; });
    try { final s = await _api.mine(); if (mounted) setState(() { _stores = s; _loadingStores = false; }); }
    catch (e) { if (mounted) setState(() { _storesError = DioClient.friendly(e); _loadingStores = false; }); }
  }
  ...
  Future<void> _editProfile() async {
    ...
    await showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(28))), builder: (ctx) => Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(ctx).viewInsets.bottom), child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      ElevatedButton(onPressed: () async {
        ...
        try {
          await app.updateProfile(name: nc.text, address: ac.text, lat: lat, lng: lng);
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Profil tersimpan')));
        } catch (e) {
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gagal menyimpan profil: ${DioClient.friendly(e)}')));
        }
  ...
  @override Widget build(BuildContext ctx) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        if (_storesError != null) Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [Text(_storesError!, textAlign: TextAlign.center, style: const TextStyle(color: Colors.grey)), const SizedBox(height: 12), OutlinedButton.icon(onPressed: _loadStores, icon: const Icon(Icons.refresh), label: const Text('Coba Lagi'))]))),
        ..._stores.map((s) => Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 6), leading: CircleAvatar(radius: 26, backgroundColor: Colors.green.shade50, child: Text(avatarInitials(s.name, maxChars: 1), style: TextStyle(color: Colors.green.shade700, fontWeight: FontWeight.w800, fontSize: 16))), title: Row(children: [Text(s.name, style: const TextStyle(fontWeight: FontWeight.w700)), if (s.isVerified) const Padding(padding: EdgeInsets.only(left: 4), child: Icon(Icons.verified, size: 16, color: Colors.green))]), subtitle: Text('⭐ ${s.ratingAvg.toStringAsFixed(1)} · ${s.reviewsCount} ulasan', style: const TextStyle(fontSize: 12)), trailing: const Icon(Icons.chevron_right), onTap: () => ctx.push('/store/${s.id}/dashboard')))),
        if (_stores.isEmpty && !_loadingStores && _storesError == null) Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), child: Padding(padding: const EdgeInsets.all(32), child: Center(child: Column(children: [
        ...
        const SizedBox(height: 20),
        ListTile(leading: const Icon(Icons.logout, color: Colors.red, size: 22), title: const Text('Keluar', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w600)), onTap: _confirmLogout),
