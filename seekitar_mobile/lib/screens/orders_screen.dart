...
class _OrdersScreenState extends State<OrdersScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _list(List<Order> orders, bool seller) => orders.isEmpty ? const Center(child: Text('Belum ada pesanan', style: TextStyle(color: Colors.grey))) : RefreshIndicator(onRefresh: _load, child: ListView.builder(itemCount: orders.length, itemBuilder: (_, i) {
    final o = orders[i];
    return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 5), child: InkWell(borderRadius: BorderRadius.circular(24), onTap: () async { await ctx.push('/order-detail', extra: o); if (mounted) _load(); }, child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
...
class OrdDetail extends StatefulWidget { final Order order; const OrdDetail({super.key, required this.order}); @override State<OrdDetail> createState() => _OrdDetailState(); }
class _OrdDetailState extends State<OrdDetail> {
  final _api = ApiProvider(); bool _processing = false;
  ...
  Future<void> _uploadProof() async {
    ...
    if (x != null) {
      setState(() => _processing = true);
      try { await _api.uploadPaymentProof(widget.order.id, File(x.path)); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Bukti pembayaran terkirim!'))); }
  ...
  Future<void> _dispute() async {
    ...
    if (show != null && mounted) {
      try { await _api.disputeOrder(widget.order.id, show); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Sengketa diajukan. Admin akan meninjau dalam 1x24 jam.'))); }
      catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gagal: $e'))); }
    }
  }
  ...
  @override Widget build(BuildContext ctx) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Pesanan #${(o.orderNumber ?? o.id).substring(0, 8)}'), actions: [IconButton(icon: const Icon(Icons.discount_outlined), tooltip: 'Kupon', onPressed: () => Navigator.push(ctx, MaterialPageRoute(builder: (_) => CouponScreen(orderTotal: o.totalAmount, orderId: o.id))))]),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        if (o.status == 'menunggu_konfirmasi' || o.status == 'diproses') ...[
          Padding(padding: const EdgeInsets.only(top: 16), child: ElevatedButton.icon(onPressed: _uploadProof, icon: const Icon(Icons.receipt_long), label: const Text('Upload Bukti Pembayaran'), style: ElevatedButton.styleFrom(backgroundColor: Colors.orange, minimumSize: const Size(double.infinity, 48)))),
          ...
          Row(children: [
            ...
            const SizedBox(width: 12), Expanded(child: OutlinedButton(onPressed: _processing ? null : () => _update('selesai'), child: Text(_processing ? '...' : 'Selesai'))),
          ]),
        ],
        ...
        Padding(padding: const EdgeInsets.only(top: 10), child: OutlinedButton.icon(onPressed: _dispute, icon: const Icon(Icons.report_problem_outlined, size: 20), label: const Text('Laporkan Masalah / Sengketa'), style: OutlinedButton.styleFrom(foregroundColor: Colors.red, minimumSize: const Size(double.infinity, 48)))),
      ]),
    );
  }
