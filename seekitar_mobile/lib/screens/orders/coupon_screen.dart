...
class _CouponScreenState extends State<CouponScreen> {
  ...
  Future<void> _validate() async {
    ...
    try { final r = await _api.validateCoupon(_code.text, widget.orderTotal); setState(() => _result = r); }
  ...
  @override Widget build(BuildContext ctx) => Scaffold(
    ...
    body: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
      ...
      if (_result != null) ...[const SizedBox(height: 16), Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
        ...
        Text('Diskon: Rp ${_result!['discount']}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF168A4A))),
        const SizedBox(height: 16),
        SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _apply, child: const Text('Pakai Kupon'))),
      ])))],
    ])),
  );
